...
/**
 * Refresh now and return the fresh records in the result.
 *
 * This is the path the dashboard uses, and returning the data matters as much
 * as fetching it. The previous design wrote to disk and had the client re-read
 * it through a second request — which breaks on serverless, where the write
 * lands in one instance's /tmp and the read can be served by another that still
 * sees the snapshot committed with the deployment. Carrying the records in the
 * response makes the result independent of where it is served from.
 *
 * Both platforms run through Promise.all; a rejection cannot escape because
 * scrapeAndSave contains its own errors.
 *
 * @param {string[]} [only]
 * @returns {Promise<{totalMs, platforms: Record<string, object>}>}
 */
async function refreshNow(only = null) {
  const names = ORDER.filter((name) => SPECS[name] && (!only || only.includes(name)));
  const startedAt = Date.now();

  // No browser is launched for API-backed platforms.
  const browser = names.some((name) => needsBrowser(name)) ? await launchBrowser() : null;
  ...
  try {
    const results = await Promise.all(
      names.map(async (name) => {
        ...
        return [
          name,
          {
            state: result.ok ? OUTCOME.SUCCESS : classify(result),
            ms: Date.now() - began,
            changed: Boolean(result.changed),
            error: result.error || null,
            status: result.record?.status ?? null,
            // The fresh record travels with the response.
    ...
    return { totalMs: Date.now() - startedAt, platforms: Object.fromEntries(results) };
  } finally {
    if (browser) await browser.close().catch(() => {});
    await shutdownOcr();
  }
...
module.exports = { getState, startRefresh, runRefresh, refreshNow, OUTCOME, PLATFORM_TIMEOUT_MS };
