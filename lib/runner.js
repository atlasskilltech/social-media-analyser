...
async function fetchAndSave(spec) {
  ...
  try {
    ...
    if (!saved.written) {
      ...
      return { ok: false, platform, record, report, forensics: null, error: saved.reason };
    }

    await logEvent(platform, 'SUCCESS', `via ${source || 'api'} in ${scrapeTime}ms`);
    console.log(`[api] ${platform} ok via ${source || 'api'} (${scrapeTime}ms)`);

    return { ok: true, platform, record: saved.data, report, forensics: null };
  } catch (err) {
