...
module.exports = {
  ...
  /*
   * Fixed pipeline order. The first strategy that satisfies requiredKeys wins
   * and the rest are never reached — see lib/pipeline.js.
   *
   * The two legacy payload readers (__additionalDataLoaded, _sharedData) sit
   * beside embedded-json because they are the same class of source; Instagram
   * removed both years ago, so in practice they log a miss and cost nothing.
   */
  strategies: [
    ...
    // rendered page was bounced to a login wall. This is the strategy that
    // works on Vercel; it must stay ahead of OCR, which cannot read a login page.
    { name: 'raw-html-fetch', run: fromRawHtml, needsPage: false },
    createOcrStrategy('instagram', (text) => {
      const handle = text.match(/@([A-Za-z0-9._]{3,})/);
      return { username: field(handle ? handle[1] : null) };
    }),
  ],
};
