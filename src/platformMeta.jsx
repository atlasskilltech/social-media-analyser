/**
 * Per-platform presentation config.
 *
 * This is the only place that knows a platform exists. PlatformCard reads from
 * here, so wiring up YouTube means filling in the entry below — no new
 * component, no changes to the card, no branching in App.
 *
 * `stats` maps the JSON keys each scraper writes onto card labels.
 */

const InstagramIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23-.22.56-.48.96-.9 1.38-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41-.56-.22-.96-.48-1.38-.9-.42-.42-.68-.82-.9-1.38-.16-.42-.36-1.06-.41-2.23-.06-1.27-.07-1.65-.07-4.85s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41C8.42 2.17 8.8 2.16 12 2.16M12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63c-.79.31-1.46.72-2.13 1.38C1.35 2.68.94 3.35.63 4.14.33 4.9.13 5.78.07 7.05.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.31.79.72 1.46 1.38 2.13.67.66 1.34 1.07 2.13 1.38.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56.79-.31 1.46-.72 2.13-1.38.66-.67 1.07-1.34 1.38-2.13.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91-.31-.79-.72-1.46-1.38-2.13-.67-.66-1.34-1.07-2.13-1.38-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0z" />
    <path d="M12 5.84a6.16 6.16 0 100 12.32 6.16 6.16 0 000-12.32zM12 16a4 4 0 110-8 4 4 0 010 8z" />
    <circle cx="18.41" cy="5.59" r="1.44" />
  </svg>
);

const YouTubeIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M23.5 6.19a3.02 3.02 0 00-2.12-2.14C19.5 3.55 12 3.55 12 3.55s-7.5 0-9.38.5A3.02 3.02 0 00.5 6.19C0 8.08 0 12 0 12s0 3.92.5 5.81a3.02 3.02 0 002.12 2.14c1.88.5 9.38.5 9.38.5s7.5 0 9.38-.5a3.02 3.02 0 002.12-2.14C24 15.92 24 12 24 12s0-3.92-.5-5.81zM9.55 15.57V8.43L15.82 12l-6.27 3.57z" />
  </svg>
);

const FacebookIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.96.93-1.96 1.89v2.25h3.33l-.53 3.49h-2.8V24C19.61 23.1 24 18.1 24 12.07z" />
  </svg>
);

const LinkedInIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.03-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.48-.9 1.63-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.46v6.28zM5.34 7.43a2.06 2.06 0 110-4.13 2.06 2.06 0 010 4.13zM7.12 20.45H3.56V9h3.56v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.72v20.56C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.72V1.72C24 .77 23.2 0 22.22 0z" />
  </svg>
);

export const PLATFORM_META = {
  instagram: {
    icon: InstagramIcon,
    color: '#E1306C',
    accent: 'linear-gradient(135deg, #833AB4 0%, #E1306C 55%, #F77737 100%)',
    /** Primary heading on the card. */
    title: (d) => (d.username ? `@${d.username}` : 'Instagram'),
    /** Secondary line under the heading. */
    subtitle: (d) => d.bio,
    stats: [
      { label: 'Followers', key: 'followers' },
      { label: 'Following', key: 'following' },
      { label: 'Posts', key: 'posts' },
    ],
  },

  youtube: {
    icon: YouTubeIcon,
    color: '#FF0000',
    accent: 'linear-gradient(135deg, #FF0000 0%, #CC0000 100%)',
    title: (d) => d.channelName || 'YouTube',
    subtitle: (d) => d.description,
    stats: [
      { label: 'Subscribers', key: 'subscribers' },
      { label: 'Videos', key: 'videos' },
    ],
  },

  facebook: {
    icon: FacebookIcon,
    color: '#1877F2',
    accent: 'linear-gradient(135deg, #1877F2 0%, #0C5DC4 100%)',
    title: (d) => d.pageName || 'Facebook',
    subtitle: (d) => d.category,
    stats: [
      { label: 'Followers', key: 'followers' },
      { label: 'Likes', key: 'likes' },
    ],
  },

  linkedin: {
    icon: LinkedInIcon,
    color: '#0A66C2',
    accent: 'linear-gradient(135deg, #0A66C2 0%, #084C94 100%)',
    title: (d) => d.companyName || 'LinkedIn',
    subtitle: (d) => d.industry,
    stats: [{ label: 'Followers', key: 'followers' }],
  },
};
